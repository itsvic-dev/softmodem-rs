...
- During ANSam, 150 bits in V.21 channel 1 without the synchronisation of
  a CM count as a V.21 caller's mark or data, and the answering modem goes
  on in V.21 (§ 8.2.2). The synchronisation counts even when the octets
  after it are spoiled, as by a lost frame, so a CM that cannot be read
  once does not turn a V.8 caller into a V.21 one.
